resource "signalfx_detector" "heartbeat" {
  ...
  program_text = <<-EOF
    ...
    signal = data('cpu.utilization', extrapolation="${var.extrapolation}", filter=(not filter('aws_state', '{Code: 32,Name: shutting-down', '{Code: 48,Name: terminated}', '{Code: 62,Name: stopping}', '{Code: 80,Name: stopped}')) and (not filter('gcp_status', '{Code=3, Name=STOPPING}', '{Code=4, Name=TERMINATED}')) and (not filter('azure_power_state', 'PowerState/stopping', 'PowerState/stoppped', 'PowerState/deallocating', 'PowerState/deallocated')) and ${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "cpu" {
  ...
  program_text = <<-EOF
    signal = data('cpu.utilization', extrapolation="${var.extrapolation}", filter=${module.filter-tags.filter_custom})${var.cpu_aggregation_function}.${var.cpu_transformation_function}(over='${var.cpu_transformation_window}').publish('signal')
...
resource "signalfx_detector" "load" {
  ...
  program_text = <<-EOF
    signal = data('load.midterm', extrapolation="${var.extrapolation}", filter=${module.filter-tags.filter_custom})${var.load_aggregation_function}.${var.load_transformation_function}(over='${var.load_transformation_window}').publish('signal')
...
resource "signalfx_detector" "disk_space" {
  ...
  program_text = <<-EOF
    signal = data('disk.utilization', extrapolation="${var.extrapolation}", filter=${module.filter-tags.filter_custom})${var.disk_space_aggregation_function}.${var.disk_space_transformation_function}(over='${var.disk_space_transformation_window}').publish('signal')
...
resource "signalfx_detector" "disk_inodes" {
  ...
  program_text = <<-EOF
    signal = data('percent_inodes.used', extrapolation="${var.extrapolation}", filter=${module.filter-tags.filter_custom})${var.disk_inodes_aggregation_function}.${var.disk_inodes_transformation_function}(over='${var.disk_inodes_transformation_window}').publish('signal')
...
resource "signalfx_detector" "disk_running_out" {
  ...
  program_text = <<-EOF
    ...
    signal = data('disk.utilization', extrapolation="${var.extrapolation}", filter=${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "memory" {
  ...
  program_text = <<-EOF
    signal = data('memory.utilization', extrapolation="${var.extrapolation}", filter=${module.filter-tags.filter_custom})${var.memory_aggregation_function}.${var.memory_transformation_function}(over='${var.memory_transformation_window}').publish('signal')
