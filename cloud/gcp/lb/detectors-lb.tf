...
resource "signalfx_detector" "request_count" {
  ...
  program_text = <<-EOF
    signal = data('https/request_count', filter=filter('bucket', 'loadbalancing') and ${module.filter-tags.filter_custom}, extrapolation='last_value', rollup='sum')${var.request_count_aggregation_function}.rateofchange()${var.request_count_transformation_function}.publish('signal')
